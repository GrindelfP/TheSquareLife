...
using static SquareLife.EntitySize;
namespace SquareLife
{
    internal class Population
    {
        ...
        {
            var minSize = KUVAHAKU_SIZE + KUVAT_SIZE + UUTISET_SIZE + 2;
            ...
            var numberOfEntities = 1 + numberOfKuvahakus + numberOfuvatus; // This and folowing is not used! Why? Check!
            var numberOfFreeAreas = (BoardSize.NumberOfRows * BoardSize.NumberOfColumns) / (MIN_ENTITY_AREA_SIZE * MIN_ENTITY_AREA_SIZE);
            var areas = new List<Coordinate>();
            for (var rowIndex = 1; rowIndex < BoardSize.NumberOfRows; rowIndex += MIN_ENTITY_AREA_SIZE)
            {
                for (var positionInRow = 1; positionInRow < BoardSize.NumberOfColumns; positionInRow += MIN_ENTITY_AREA_SIZE)
                {
